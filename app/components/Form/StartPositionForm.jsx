...
const StartPositionForm = ({
  ...
  const handleNextPage = () => {
    onNextPage({
      start_condition: localData.start_condition,
    });
  };
  ...
  const handleAddToast = (condition) => {
    setLocalData((prevState) => {
      const newToast = {
        ...
        order: prevState.start_condition.length,
      };

      return {
        ...prevState,
        start_condition: [...prevState.start_condition, newToast],
      };
    });
  };
  ...
  const handleCloseToast = (id) => {
    setLocalData((prevState) => {
      const updatedToasts = prevState.start_condition.filter(
        (toast) => toast.id !== id
      );
      return {
        ...prevState,
        start_condition: updatedToasts,
      };
    });
  };
  ...
  const handleInputChange = (id, field, value) => {
    setLocalData((prevState) => {
      const updatedToasts = prevState.start_condition.map((toast) => {
        if (toast.id === id) {
          return {
            ...toast,
            formData: {
        ...
        <div className="right-side min-w-1/2 flex-1 grid gap-4 mb-4">
          <div className="col-span-1 w-full overflow-y-auto max-h-[450px] pl-4">
            ...
            <div className="flex flex-wrap">
              ...
              {localData.start_condition.map((toast) => renderToastForm(toast))}
            </div>
          </div>
        </div>
      </div>
